...
from utilities import file_path_PARPML_6inputs_W
...
os.makedirs(save_dir_PARPML_6inputs_W, exist_ok=True)

# Load the CSV file into a DataFrame
data = pd.read_csv(file_path_PARPML_6inputs_W)
...
for model_name, model in models.items():
    ...
    plot_filtered_predictions_vs_ground_truth(model, X_train, y_train, y_test, y_test_pred, model_name, export_figure,
                                              y_range=y_range, save_dir=save_dir_PARPML_6inputs_W, model_specific_name='PARPML')
    ...
    append_model_to_file(model_name,num_param, mse_avg, mse_var, r2_avg, r2_var, mse_test, r2_test,save_dir_PARPML_6inputs_W, 'RPML_model_comparison_table.txt', train_time=train_time)

# Save each trained model
for model_name, model in models.items():
    save_path = os.path.join(save_dir_PARPML_6inputs_W, f"{model_name.replace(' ', '_')}_model.pkl")
